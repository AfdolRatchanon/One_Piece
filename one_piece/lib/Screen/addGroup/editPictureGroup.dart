...
class editPictureGroup extends StatefulWidget {
  const editPictureGroup({Key? key}) : super(key: key);

  @override
  _editPictureGroupState createState() => _editPictureGroupState();
}

class _editPictureGroupState extends State<editPictureGroup> {
    ...
    if (file != null) {
      ...
      try {
        TaskSnapshot snapshot = await storage
            .ref()
            .child("pictureGroup/${keyIn.name}")
            .putFile(file);
        if (snapshot.state == TaskState.success) {
          final String downloadUrl = await snapshot.ref.getDownloadURL();
          await db.child(keyIn.key).update({
